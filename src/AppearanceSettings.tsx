...
export default function AppearanceSettings({ appearance, onChange, onClose, storageError }: {
  ...
  function commitHex() {
    ...
    if (!value) { setError('Enter a six-digit hex color, such as #5799ed.'); return false; }
    setError(''); setHex(value); onChange({ ...appearance, accent: value }); return true;
  }
  return <div className="appearance-settings">
    ...
    <fieldset><legend>Accent color</legend><div className="accent-options">
      ...
      <div className="accent-error-space">{error && <p id="accent-error" className="settings-error" role="alert">{error}</p>}</div>
    ...
    <div className="settings-footer"><button type="button" className="secondary-button" onClick={() => { setHex(DEFAULT_APPEARANCE.accent); setError(''); onChange({ ...DEFAULT_APPEARANCE }); }}>Reset appearance</button><button type="button" className="primary-button" onClick={() => { if (commitHex()) onClose(); }}>Done</button></div>
  </div>;
}
