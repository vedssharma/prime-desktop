...
function PrimeMark({ className = '' }: { className?: string }) { return <svg className={className} viewBox="0 0 32 32" fill="none" aria-hidden="true"><path d="M16 2v28M2 16h28M6.1 6.1l19.8 19.8M6.1 25.9L25.9 6.1" stroke="currentColor" strokeWidth="5" /></svg>; }
...
export default function App() {
  ...
  return <div className={`app-shell ${isElectron ? 'electron' : 'browser-preview'}`}>
    ...
    <aside className={`sidebar ${sidebarOpen ? 'is-open' : ''}`}>
      <div className="window-drag"><div className="window-dots" aria-hidden="true"><i /><i /><i /></div><span>PRIME DESKTOP</span></div>
      <div className="brand"><PrimeMark /><span>prime<span className="brand-period">.</span></span><span className="desktop-badge">desktop</span><button className="icon-button mobile-close" aria-label="Close sidebar" onClick={() => setSidebarOpen(false)}><PanelLeftClose size={17} /></button></div>
      ...
      <div className="sidebar-bottom"><div className="local-note"><Terminal size={15} /><div><strong>Your machine. Your flow.</strong><span>Powered by Prime Agent CLI</span></div></div><button className="connection-button" onClick={reconnect} disabled={connecting} title={connection?.error || 'Reconnect to Prime Agent'}><span className={`status-dot ${connection?.connected ? 'connected' : ''}`} /><span>{connecting ? 'Connecting...' : connection?.connected ? 'Agent connected' : connection ? 'Agent disconnected' : 'Checking connection...'}</span>{connecting ? <LoaderCircle size={13} className="spin" /> : <RefreshCw size={13} />}</button><div className="sidebar-footer"><span>PRIME INTELLECT</span><button className="icon-button" aria-label="About Prime Desktop" onClick={() => setDialog('about')}><CircleHelp size={16} /></button></div></div>
    </aside>
